...
int main(int argc, char ** argv)
{
    ...
    string str_file = basename(const_cast<char*>(filename));
    string ast_file = basename(const_cast<char*>(filename));
    ...
    str_file = str_file.substr(0, str_file.length() - 3) + ".str";
    ast_file = ast_file.substr(0, ast_file.length() - 3) + ".ast"; 
    
    FILE * str_output = fopen(str_file.c_str(), "w");
    FILE * ast_output = fopen(ast_file.c_str(), "w");

    string_set::dump(str_output);
    astree::print(ast_output, parser::root, 0);
